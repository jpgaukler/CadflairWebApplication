        ...
        public static void ConvertiLogicFormButton_OnExecute(NameValueMap Context)
        {
            ...
            try
            {
                foreach (AttributeSet set in Globals.InventorApplication.ActiveDocument.AttributeSets)
                {
                    ...
                    string jsonString = SerializeiLogicFormSpecToJson(Globals.InventorApplication.ActiveDocument, xmlString);
                    ...
                    string jsonFileName = System.IO.Path.Combine(folderName, formName + ".json");

                    StreamWriter xmlFile = System.IO.File.CreateText(xmlFileName);
                    StreamWriter htmlFile = System.IO.File.CreateText(jsonFileName);

                    htmlFile.Write(jsonString);
        ...
        private static ILogicUiElement[] RecurseILogicElements(Document doc, XElement itemsElement, XNamespace ns)
        {
            ...
            foreach (XElement item in itemsElement.Elements())
            {
                ...
                ILogicUiElement element = new ILogicUiElement()
                {
                    ...
                    ParameterValue = doc.GetParameter(item.Element("ParameterName")?.Value)?.Expression,
                    //ParameterMinValue = doc.GetParameter(item.Element("ParameterName")?.Value).Tolerance.Lower.ToString(),
