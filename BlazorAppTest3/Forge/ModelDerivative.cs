...
{
    public static class ModelDerivative 
    {

        #region Fields

        private static readonly DerivativesApi _derivativesApi = new(); 
        ...
        {
            dynamic oauth = OAuth.GetInternal();
            string? objectUrn = await ObjectStorage.GetEncodedUrnAsync(bucketKey, objectKey);
        ...
        /// <returns></returns>
        public static async Task<string> GetThumbnailBase64String(string base64Urn)
        {
            try
            {
